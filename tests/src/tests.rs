use super::common::{
    eth160, DummyDataLoader, KECCAK256_ALL_ACPL_BIN, MAX_CYCLES, SECP256K1_DATA_BIN,
};
use ckb_tool::ckb_crypto::secp::Generator;
use ckb_tool::ckb_error::assert_error_eq;
use ckb_tool::ckb_script::{ScriptError, TransactionScriptsVerifier};
use ckb_tool::ckb_types::core::cell::{CellMetaBuilder, ResolvedTransaction};
use ckb_tool::ckb_types::core::{
    Capacity, DepType, ScriptHashType, TransactionBuilder, TransactionView,
};
use ckb_tool::ckb_types::packed::{
    CellDep, CellInput, CellOutput, OutPoint, Script, WitnessArgsBuilder,
};
...
fn test_put_output_data() {
    ...
    assert_error_eq!(
        ...
        ScriptError::ValidationFailure(ERROR_ENCODING).input_lock_script(0),
    );
}
...
fn test_wrong_output_args() {
    ...
    assert_error_eq!(
        verify_result.unwrap_err(),
        ScriptError::ValidationFailure(ERROR_NO_PAIR).input_lock_script(0),
    );
}
...
fn test_split_cell() {
    ...
    assert_error_eq!(
        verify_result.unwrap_err(),
        ScriptError::ValidationFailure(ERROR_DUPLICATED_OUTPUTS).input_lock_script(0),
    );
}
...
fn test_merge_cell() {
    ...
    assert_error_eq!(
        verify_result.unwrap_err(),
        ScriptError::ValidationFailure(ERROR_DUPLICATED_INPUTS).input_lock_script(0),
    );
}
...
fn test_insufficient_pay() {
    ...
    assert_error_eq!(
        verify_result.unwrap_err(),
        ScriptError::ValidationFailure(ERROR_OUTPUT_AMOUNT_NOT_ENOUGH).input_lock_script(0),
    );
}
